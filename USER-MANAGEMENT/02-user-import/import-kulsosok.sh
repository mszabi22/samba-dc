...
echo "$surname $givenname külsős hozzáadása..."
    samba-tool user create --must-change-at-next-login --script-path="tanar.cmd" --surname="$surname" --given-name="$givenname" --mail-address="$username@crnl.hu" --department="Külsős" $username $JELSZO
