...
echo "$surname $givenname kollégiumi dolgozó hozzáadása..."
    samba-tool user create --must-change-at-next-login --script-path="kollegium.cmd" --surname="$surname" --given-name="$givenname" --mail-address="$username@crnl.hu" --department="Kollégium" $username $JELSZO
